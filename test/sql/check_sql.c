...
static M_sql_error_t check_sql_trans(M_sql_trans_t *trans, void *arg, char *error, size_t error_size)
{
	...
	for (i=0; i<INSERT_ROWS; i++) {
		...
		M_snprintf(temp, sizeof(temp), "Row%"PRIu64"", (M_uint64)i+1);
...
START_TEST(check_sql)
{
	...
	for (i=0; i<2; i++) {
		...
		M_snprintf(temp, sizeof(temp), "Row%"PRIu64"", (M_uint64)INSERT_ROWS+i+1);
		M_sql_stmt_bind_text_dup(stmt, temp, 0);
...
#define HUGEDATA_SIZE (1024 * 1024)
	...
	M_snprintf(temp, sizeof(temp), "Row%"PRIu64"", (M_uint64)INSERT_ROWS+5+1);
	M_sql_stmt_bind_text_dup(stmt, temp, 0);
	...
	ck_assert_msg(M_sql_stmt_result_num_cols(stmt) == 8, "M_sql_stmt_result_num_cols() expected 8, got %"PRIu64"", (M_uint64)M_sql_stmt_result_num_cols(stmt));
...
#if defined(DEBUG) && DEBUG >= 1
	M_printf("%"PRIu64" cols (", (M_uint64)M_sql_stmt_result_num_cols(stmt));
	...
	ck_assert_msg(M_csv_get_numrows(csv) == M_sql_stmt_result_total_rows(stmt), "mismatch between csv rows and sql rows: %"PRIu64" vs %"PRIu64"", (M_uint64)M_csv_get_numrows(csv), (M_uint64)M_sql_stmt_result_total_rows(stmt));
	ck_assert_msg(M_csv_get_numcols(csv) == M_sql_stmt_result_num_cols(stmt), "mismatch between csv cols and sql cols: %"PRIu64" vs %"PRIu64"", (M_uint64)M_csv_get_numcols(csv), (M_uint64)M_sql_stmt_result_num_cols(stmt));
	...
	ck_assert_msg(outbincol_size == HUGEDATA_SIZE, "Expected huge binary column to be %"PRIu64" bytes, was %"PRIu64" bytes", (M_uint64)HUGEDATA_SIZE, (M_uint64)outbincol_size);
	for (i=0; i<outbincol_size; i++) {
		ck_assert_msg(outbincol[i] == 0x0D, "Binary data index %"PRIu64" (0x%02X) does not match expected value of 0x0D", (M_uint64)i, outbincol[i]);
	}
	M_sql_stmt_destroy(stmt);
