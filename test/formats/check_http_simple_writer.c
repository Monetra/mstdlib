...
static void validate_output(char *out, size_t *out_len, const char *expected, size_t idx)
{
	...
	ck_assert_msg(M_str_eq(out, expected), "%"PRIu64": output does not match expected.\nGot:\n'%s'\n--\nExpected\n'%s'\n", (M_uint64)idx, out, expected);
	...
	ck_assert_msg(res == M_HTTP_ERROR_SUCCESS || res == M_HTTP_ERROR_MOREDATA, "%"PRIu64": Could not read output", (M_uint64)idx);
}
...
START_TEST(check_request)
{
	...
	for (i=0; params[i].method!=M_HTTP_METHOD_UNKNOWN; i++) {
		...
		if (params[i].out == NULL) {
			ck_assert_msg(out == NULL, "%"PRIu64": output structured when expected failure.\nGot:\n'%s'\n", (M_uint64)i, out);
			continue;
		}
