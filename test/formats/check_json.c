...
START_TEST(check_json_valid)
{
	...
	for (i=0; check_json_valid_data[i].data!=NULL; i++) {
		...
		ck_assert_msg(json != NULL, "JSON (%" PRIu64 ") '%s' could not be parsed: %d", (M_uint64)i, check_json_valid_data[i].data, error);

		if (check_json_valid_data[i].out != NULL) {
			out = M_json_write(json, check_json_valid_data[i].writer_flags, NULL);
			ck_assert_msg(M_str_eq(out, check_json_valid_data[i].out), "Output not as expected (%" PRIu64 "):\ngot='%s'\nexpected='%s'", (M_uint64)i, out, check_json_valid_data[i].out);
			M_free(out);
		}
...
START_TEST(check_json_invalid)
{
	...
	for (i=0; check_json_invalid_data[i].data!=NULL; i++) {
		...
		ck_assert_msg(json == NULL, "Invalid JSON was parsed (%" PRIu64 "): %s", (M_uint64)i, check_json_invalid_data[i].data);
		if (check_json_invalid_data[i].error_line != 0 && check_json_invalid_data[i].error_pos != 0)
			ck_assert_msg(check_json_invalid_data[i].error_line == error_line && check_json_invalid_data[i].error_pos == error_pos, "Parse error (%" PRIu64 ") '%s' was not found at expected location. Found. %" PRIu64 ":%" PRIu64 ". Expected: %" PRIu64 ":%" PRIu64 "", (M_uint64)i, check_json_invalid_data[i].data, (M_uint64)error_line, (M_uint64)error_pos, (M_uint64)check_json_invalid_data[i].error_line, (M_uint64)check_json_invalid_data[i].error_pos);
		M_json_node_destroy(json);
	}
}
END_TEST
...
START_TEST(check_json_reader_flags)
{
	...
	for (i=0; check_json_reader_flags_data[i].data!=NULL; i++) {
		...
		if (check_json_reader_flags_data[i].will_error == M_TRUE) {
			ck_assert_msg(json == NULL, "Invalid JSON was parsed (%" PRIu64 "): %s", (M_uint64)i, check_json_reader_flags_data[i].data);
			if (check_json_reader_flags_data[i].error_line != 0 && check_json_reader_flags_data[i].error_pos != 0)
				ck_assert_msg(check_json_reader_flags_data[i].error_line == error_line && check_json_reader_flags_data[i].error_pos == error_pos, "Parse error (%" PRIu64 ") '%s' was not found at expected location. Found. %" PRIu64 ":%" PRIu64 ". Expected: %" PRIu64 ":%" PRIu64 "", (M_uint64)i, check_json_reader_flags_data[i].data, (M_uint64)error_line, (M_uint64)error_pos, (M_uint64)check_json_reader_flags_data[i].error_line, (M_uint64)check_json_reader_flags_data[i].error_pos);
		} else {
			ck_assert_msg(json != NULL, "JSON (%" PRIu64 ") '%s' could not be parsed: %d", (M_uint64)i, check_json_reader_flags_data[i].data, error);
			if (check_json_reader_flags_data[i].out != NULL) {
				out = M_json_write(json, M_JSON_WRITER_NUMBER_NOCOMPAT, NULL);
				ck_assert_msg(M_str_eq(out, check_json_reader_flags_data[i].out), "Output not as expected (%" PRIu64 "):\ngot='%s'\nexpected='%s'", (M_uint64)i, out, check_json_reader_flags_data[i].out);
				M_free(out);
			}
		}
		M_json_node_destroy(json);
...
START_TEST(check_json_jsonpath_book)
{
	...
	ck_assert_msg(json != NULL, "JSONPath books string could not be parsed: %d, %" PRIu64 ":%" PRIu64 "", error, (M_uint64)error_line, (M_uint64)error_pos);
	...
	for (i=0; check_json_jsonpath_book_data[i].search!=NULL; i++) {
		...
		ck_assert_msg(results != NULL, "No matches found (%" PRIu64 "): '%s'", (M_uint64)i, check_json_jsonpath_book_data[i].search);
		ck_assert_msg(num_matches == check_json_jsonpath_book_data[i].num_matches, "Unexpected matches found (%" PRIu64 "): '%s'. Got %" PRIu64 ", expected %" PRIu64 " matches", (M_uint64)i, check_json_jsonpath_book_data[i].search, (M_uint64)num_matches, (M_uint64)check_json_jsonpath_book_data[i].num_matches);
		...
		if (results != NULL && check_json_jsonpath_book_data[i].type != M_JSON_TYPE_UNKNOWN) {
			for (j=0; j<num_matches; j++) {
				...
				ck_assert_msg(type == check_json_jsonpath_book_data[i].type, "Unexpected type (%" PRIu64 "): got %d, expected %d", (M_uint64)i, type, check_json_jsonpath_book_data[i].type);
			}
		}
...
START_TEST(check_json_jsonpath_str)
{
	...
	ck_assert_msg(json != NULL, "JSONPath string could not be parsed: %d, %" PRIu64 ":%" PRIu64 "", error, (M_uint64)error_line, (M_uint64)error_pos);
	...
	for (i=0; check_json_jsonpath_str_data[i].search!=NULL; i++) {
		...
		if (check_json_jsonpath_str_data[i].str == NULL) {
			ck_assert_msg(results == NULL, "Matches found (%" PRIu64 ") when there shouldn't be: '%s'", (M_uint64)i, check_json_jsonpath_str_data[i].search);
			continue;
		}
		ck_assert_msg(results != NULL, "No matches found (%" PRIu64 "): '%s'", (M_uint64)i, check_json_jsonpath_str_data[i].search);
		ck_assert_msg(num_matches == 1, "Unexpected matches found (%" PRIu64 "): '%s': got %" PRIu64 " matches", (M_uint64)i, check_json_jsonpath_str_data[i].search, (M_uint64)num_matches);

		/* Silence clang warning using the if != NULL thoug the test won't get this far due to the ck_assert_msg for
 		 * NULL above. */
		if (results != NULL) {
			type = M_json_node_type(results[0]);
			ck_assert_msg(type == M_JSON_TYPE_STRING, "(%" PRIu64 ") Search '%s' did not return string match", (M_uint64)i, check_json_jsonpath_str_data[i].search);
			if (type == M_JSON_TYPE_STRING) {
				str  = M_json_get_string(results[0]);
				ck_assert_msg(M_str_eq(check_json_jsonpath_str_data[i].str, str), "(%" PRIu64 ") Search '%s': did not find expected node. Got '%s', expected '%s'", (M_uint64)i, check_json_jsonpath_str_data[i].search, str, check_json_jsonpath_str_data[i].str);
			}
		}
...
START_TEST(check_json_jsonpath_array)
{
	...
	ck_assert_msg(results != NULL && num_matches == 1, "Did not find expected match: results=%s, num_matches=%" PRIu64 "", results==NULL?"NULL":"NOT NULL", (M_uint64)num_matches);
	if (results != NULL) {
	...
	results = M_json_jsonpath(json, "$.[1]", &num_matches);
	ck_assert_msg(results != NULL && num_matches == 3, "Did not find expected match: results=%s, num_matches=%" PRIu64 "", results==NULL?"NULL":"NOT NULL", (M_uint64)num_matches);
	if (results != NULL) {
		val = M_json_get_int(results[0]);
		ck_assert_msg(val == 88, "node value %" PRId64 " != 88", val);
	...
	ck_assert_msg(results != NULL && num_matches == 4, "Did not find expected match: results=%s, num_matches=%" PRIu64 "", results==NULL?"NULL":"NOT NULL", (M_uint64)num_matches);
	if (results != NULL) {
...
START_TEST(check_json_parent_object)
{
	...
	ck_assert_msg(results != NULL && num_matches == 1, "Did not find expected match for str node: results=%s, num_matches=%" PRIu64 "", results==NULL?"NULL":"NOT NULL", (M_uint64)num_matches);
	if (results != NULL)
		zy_node = results[0];
	M_free(results);

	results = M_json_jsonpath(json, "$..zz", &num_matches);
	ck_assert_msg(results != NULL && num_matches == 1, "Did not find expected match for array node: results=%s, num_matches=%" PRIu64 "", results==NULL?"NULL":"NOT NULL", (M_uint64)num_matches);
	if (results != NULL)
		zz_node = results[0];
	M_free(results);
...
START_TEST(check_json_parent_array)
{
	...
	results = M_json_jsonpath(json, "$[0]", &num_matches);
	ck_assert_msg(results != NULL && num_matches == 1, "Did not find expected match for array node (2nd search): results=%s, num_matches=%" PRIu64 "", results==NULL?"NULL":"NOT NULL", (M_uint64)num_matches);
	if (results != NULL) {
		array_node = results[0];
		M_free(results);
		M_decimal_create(&dec);
