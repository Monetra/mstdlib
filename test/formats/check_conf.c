...
START_TEST(check_sections)
{
	...
	ck_assert_msg(M_list_str_len(sections) == 2, "wrong number of sections (want 2, have %lu)", M_list_str_len(sections));

	/* Check that each section has the correct keys with the correct values. */
	for (i=0; i<M_list_str_len(sections); i++) {
		M_snprintf(section, sizeof(section), "Section%lu", i+1);

		M_snprintf(key, sizeof(key), "%s/key%lu", section, i+1);
		M_snprintf(want_value, sizeof(want_value), "value%lu", i+1);
		...
		M_snprintf(key, sizeof(key), "%s/key%lu%lu", section, i+1, i+1);
		M_snprintf(want_value, sizeof(want_value), "value%lu%lu", i+1, i+1);
		conf_value = M_conf_get_value(conf, key);
		ck_assert_msg(M_str_eq(conf_value, want_value), "wrong section key value (want %s, have %s)", want_value, conf_value);
	}
...
START_TEST(check_no_sections)
{
	...
	sections = M_conf_get_sections(conf);
	ck_assert_msg(M_list_str_len(sections) == 0, "wrong number of sections (want 0, have %lu)", M_list_str_len(sections));
...
START_TEST(check_sections_no_multi)
{
	...
	for (i=0; i<M_list_str_len(sections); i++) {
		...
		ck_assert_msg(M_list_str_len(values) == 2, "wrong number of values for %s (have %lu, want 2)", key, M_list_str_len(values));
...
START_TEST(check_unused_single)
{
	...
	ck_assert_msg(M_list_str_len(unused) == M_hash_dict_num_keys(expected_unused),
			"mismatch in number of keys unused (expected %lu, have %lu)", M_hash_dict_num_keys(expected_unused), M_list_str_len(unused));
...
START_TEST(check_unused_multi)
{
	...
	unused = M_conf_unused_keys(conf);
	ck_assert_msg(M_list_str_len(unused) == 15, "mismatch in number of keys unused (have %lu, want 15)", M_list_str_len(unused));

	count = M_list_str_count(unused, "strdup_key", M_LIST_STR_MATCH_VAL);
	ck_assert_msg(count == 1, "strdup count wrong (want 1, have %lu", count);

	count = M_list_str_count(unused, "int8_key", M_LIST_STR_MATCH_VAL);
	ck_assert_msg(count == 2, "int8 count wrong (want 2, have %lu", count);

	count = M_list_str_count(unused, "int16_key", M_LIST_STR_MATCH_VAL);
	ck_assert_msg(count == 3, "int16 count wrong (want 3, have %lu", count);

	count = M_list_str_count(unused, "int32_key", M_LIST_STR_MATCH_VAL);
	ck_assert_msg(count == 4, "int32 count wrong (want 4, have %lu", count);

	count = M_list_str_count(unused, "int64_key", M_LIST_STR_MATCH_VAL);
	ck_assert_msg(count == 5, "int64 count wrong (want 5, have %lu", count);
