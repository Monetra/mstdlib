...
START_TEST(check_buf_uintbin)
{
	...
	for (i=0; check_buf_uintbin_data[i].n!=0; i++) {
		...
		buf = M_buf_create();
		ck_assert_msg(M_buf_add_uintbin(buf, n, num, endianness), "%lu: Could not convert '%"PRIu64"' to bin, with %s", i, n, endianness==M_ENDIAN_BIG?"BIG":"LITTLE");
		out = M_buf_finish(buf, &out_len);

		out_hex = M_bincodec_encode_alloc((M_uint8 *)out, out_len, 0, M_BINCODEC_HEX);
		M_free(out);
		ck_assert_msg(M_str_caseeq(out_hex, hex), "%lu: '%s' does not match expected '%s' with %s", i, out_hex, hex, endianness==M_ENDIAN_BIG?"BIG":"LITTLE");
...
START_TEST(check_buf_strbin)
{
	...
	for (i=0; check_buf_strbin_data[i].in!=NULL; i++) {
		...
		buf = M_buf_create();
		ck_assert_msg(M_buf_add_uintstrbin(buf, in, base, bytes, M_ENDIAN_BIG), "%lu: Could not convert '%s' to bin", i, in);
		out = M_buf_finish(buf, &out_len);

		out_hex = M_bincodec_encode_alloc((M_uint8 *)out, out_len, 0, M_BINCODEC_HEX);
		M_free(out);
		ck_assert_msg(M_str_caseeq(out_hex, hex), "%lu: '%s' does not match expected '%s'", i, out_hex, hex);
...
START_TEST(check_buf_uintbcd)
{
	...
	for (i=0; check_buf_uintbcd_data[i].n!=0; i++) {
		...
		if (ret == M_FALSE) {
			ck_assert_msg(hex == NULL, "%lu: Failed to convert %"PRIu64" to bcd", i, n);
			continue;
		}
