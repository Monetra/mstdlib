...
START_TEST(check_time_fmt)
{
	...
	for (i=0; check_fmts[i].ts!=0; i++) {
		...
		ck_assert_msg(M_str_eq(check_fmts[i].s, out), "%lu: Expected string '%s' does not match returned '%s'", i, check_fmts[i].s, out);

		ts = M_time_from_str(out, tz, M_FALSE);
		ck_assert_msg(check_fmts[i].ts == ts, "%lu: Expected ts %"PRId64" does not match returned %"PRId64"", i, (M_int64)check_fmts[i].ts, ts);
		M_free(out);

		out = M_time_to_str("%Z", &tm);
		ck_assert_msg(M_str_eq(check_fmts[i].abbr, out), "%lu: Expected abbr '%s' does not match returned '%s'", i, check_fmts[i].abbr, out);
		ck_assert_msg(M_str_eq(out, tm.abbr), "%lu: Expected abbr '%s' does not match tm.abbr '%s'", i, out, tm.abbr);
		M_free(out);
	}
...
START_TEST(check_time_fmt_custom)
{
	...
	for (i=0; check_fmts_custom[i].ts!=0; i++) {
		...
		if (check_fmts_custom[i].fail) {
			ck_assert_msg(out == NULL, "%lu: Parse succeeded when it should have failed", i);
			continue;
		} else {
			ck_assert_msg(out != NULL && *out == '\0', "%lu: Did not fully parse data: %s", i, check_fmts_custom[i].s);
		}
		ts = M_time_fromlocal(&tm, tz);
		ck_assert_msg(check_fmts_custom[i].ts == ts, "%lu: Expected ts %"PRId64" does not match returned %"PRId64"", i, check_fmts_custom[i].ts, ts);

		out = M_time_to_str(check_fmts_custom[i].tostr_fmt, &tm);
		ck_assert_msg(M_str_eq(check_fmts_custom[i].s, out), "%lu: Expected string '%s' does not match returned '%s'", i, check_fmts_custom[i].s, out);
		M_free(out);
	}
...
START_TEST(check_time_fmt_utc)
{
	...
	for (i=0; check_fmts_utc[i].ts!=0; i++) {
		...
		ck_assert_msg(check_fmts_utc[i].ts == ts, "%lu: Expected ts %"PRId64" does not match returned %"PRId64"", i, (M_int64)check_fmts_utc[i].ts, ts);
	}
}
END_TEST
...
int main(void)
{
	...
	if (getenv("CK_LOG_FILE_NAME")==NULL) srunner_set_log(sr, "check_time_fmt.log");
