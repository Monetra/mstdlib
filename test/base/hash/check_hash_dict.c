...
static void check_create_pow2_help(size_t size)
{
	...
	if (d != NULL) {
		...
		ck_assert_msg(r_size == e_size, "for %"PRIu64", expected %"PRIu64", got %"PRIu64"", (M_uint64)size, (M_uint64)e_size, (M_uint64)r_size);
...
static void ensure_size(size_t e_size)
{
	...
	ck_assert_msg(r_size == e_size, "expected %"PRIu64", got %"PRIu64"", (M_uint64)e_size, (M_uint64)r_size);
}
...
START_TEST(check_merge_different)
{
	...
	while (M_hash_dict_enumerate_next(dest, he, &key, &val)) {
		ck_assert_msg(M_str_len(key) == 3, "Key length %"PRIu64" != 3", (M_uint64)M_str_len(key));
		ck_assert_msg(M_str_len(val) == 3, "Value length %"PRIu64" != 3", (M_uint64)M_str_len(val));
	}
	M_hash_dict_enumerate_free(he);
	M_hash_dict_remove(dest, "vAl");
	ck_assert_msg(M_hash_dict_num_keys(dest) == 2, "Num keys in dest %"PRIu64" != 2", (M_uint64)M_hash_dict_num_keys(dest));
...
START_TEST(check_casesensitive)
{
	...
	for (i=0; pairs[i].key != NULL; i++)
		ck_assert_msg(M_hash_dict_insert(d, pairs[i].key, pairs[i].val), "%"PRIu64": insert failed", (M_uint64)i);
	for (i=0; pairs[i].key != NULL; i++)
		ck_assert_msg(M_str_eq(M_hash_dict_get_direct(d, pairs[i].key), pairs[i].val), "%"PRIu64": get failed", (M_uint64)i);
...
START_TEST(check_reuse_val)
{
	...
	ck_assert_msg(num_entries == 1, "expected 1, got %"PRIu64"", (M_uint64)num_entries);
...
int main(void)
{
	...
	if (getenv("CK_LOG_FILE_NAME")==NULL) srunner_set_log(sr, "check_hash_dict.log");
