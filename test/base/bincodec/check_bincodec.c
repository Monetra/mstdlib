...
START_TEST(check_bincodec_encode_alloc)
{
	...
	for (i=0; check_bincodec_encode_data[i].data!=NULL; i++) {
		...
		ck_assert_msg(out != NULL, "%lu: Could not encode", i);
		if (out == NULL)
			continue;
		ck_assert_msg(M_str_eq(out, check_bincodec_encode_data[i].out), "%lu: got='%s', expected='%s'", i, out, check_bincodec_encode_data[i].out);
		M_free(out);
	}
}
END_TEST
...
START_TEST(check_bincodec_encode)
{
	...
	for (i=0; check_bincodec_encode_data[i].data!=NULL; i++) {
		...
		ck_assert_msg(encode_len != 0 && encode_len <= out_len, "%lu: Could not encode", i);
		out[encode_len] = '\0';
		ck_assert_msg(M_str_eq(out, check_bincodec_encode_data[i].out), "%lu: got='%s', expected='%s'", i, out, check_bincodec_encode_data[i].out);
		M_free(out);
	}
...
START_TEST(check_bincodec_decode)
{
	...
	for (i=0; check_bincodec_decode_data[i].data!=NULL; i++) {
		...
		ck_assert_msg(out != NULL, "%lu: Could not decode", i);
		ck_assert_msg(M_str_eq(out, check_bincodec_decode_data[i].out), "%lu: got='%s', expected='%s'", i, out, check_bincodec_decode_data[i].out);
		M_free(out);
	}
}
END_TEST
