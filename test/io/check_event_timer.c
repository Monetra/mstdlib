                               ...
                               size_t fire_cnt, M_event_timer_mode_t mode, M_bool use_trigger, M_uint64 first_event_delay_ms)
{
	...
	event_debug("start_delay_ms=%"PRIu64", end_ms=%"PRIu64", interval_ms=%"PRIu64", max_runtime_ms=%"PRIu64", fire_cnt=%lu, mode=%s, use_trigger=%s, first_event_delay_ms=%"PRIu64"",
...
cleanup:
	...
	M_library_cleanup();
	event_debug("exited (%lu events)", events);
	return events;
}
...
int main(int argc, char **argv)
{
	...
	if (getenv("CK_LOG_FILE_NAME")==NULL) srunner_set_log(sr, "check_event_timer.log");
