	...
	switch (type) {
		...
		case M_EVENT_TYPE_WRITE:
			...
			if (mysize) {
				...
				event_debug("net client %p wrote %lu bytes (%"PRIu64" Bps)", comm, mysize - M_buf_len(data->buf), M_io_bwshaping_get_Bps(comm, client_id, M_IO_BWSHAPING_DIRECTION_OUT));
			}
	...
	switch (type) {
		...
		case M_EVENT_TYPE_READ:
			...
			if (err == M_IO_ERROR_SUCCESS) {
				event_debug("net serverconn %p read %lu bytes (%"PRIu64" Bps)", comm, M_buf_len(data->buf) - mysize, M_io_bwshaping_get_Bps(comm, server_id, M_IO_BWSHAPING_DIRECTION_IN));
//				M_printf("read size %lu bytes\n", M_buf_len(data->buf) - mysize);
