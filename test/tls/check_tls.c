...
M_printf("ServerCert: %s\n", realcert);
	...
	event_debug("%lu remaining objects", M_event_num_objects(event));
	/* Cleanup */
...
static void net_serverconn_sad_cb(M_event_t *event, M_event_type_t type, M_io_t *comm, void *data)
{
	...
	switch (type) {
		...
		case M_EVENT_TYPE_WRITE:
			...
			event_debug("net sad serverconn %p wrote %lu bytes", comm, mysize - M_buf_len(wbuf));
			if (M_buf_len(wbuf) == 0) {
				M_io_destroy(comm);
				M_buf_cancel(wbuf);
				wbuf = NULL;
...
static void net_client_sad_cb(M_event_t *event, M_event_type_t type, M_io_t *comm, void *data)
{
	...
	switch (type) {
		...
		case M_EVENT_TYPE_READ:
			...
			event_debug("net sad client %p read %lu bytes", comm, M_buf_len(rbuf) - orig_size);
			if (M_buf_len(rbuf))
		...
		case M_EVENT_TYPE_ERROR:
			if (type == M_EVENT_TYPE_ERROR) {
				...
				if (M_buf_len(rbuf) == SEND_AND_DISCONNECT_SIZE) {
					event_debug("net sad client received FULL data: %lu bytes", M_buf_len(rbuf));
					M_event_done(event);
				} else {
					event_debug("net sad client received partial data: %lu of %lu bytes", M_buf_len(rbuf), (size_t)SEND_AND_DISCONNECT_SIZE);
					M_event_return(event);
				}
			}
			M_buf_cancel(rbuf);
